...
unsigned float_i2f(int x) {
	...
	if (x) {
		...
		switch(lasc & 0xc0)
		{
			case (0x40): UnSure=1;break;
			case (0xc0): Carry=1;break;
			default: break;
		}
		if (UnSure)
		{
...
/*
	...
	if (x) {
		...
		carry = ((x & 0x80) && (x & 0x17f));
		return exp_ + (x >> 8) + carry;
	}
	return 0;
*/
/*
...
/*
	unsigned sign_ = uf & 0x80000000;
	unsigned exp_ = uf >> 23 & 0xff;
	unsigned frac_ = uf & 0x7fffff;
	unsigned ret_ = 0;
	if (exp_ < 127) return 0;
	if (exp_ >= 158) return 0x80000000;
	frac_ = frac_ | 0x800000;
	if (exp_ > 150) ret_ = frac_ << (exp_ - 150);
	else ret_ = frac_ >> (150 - exp_);
	if (sign_) ret_ = -ret_;
	return ret_;
*/
}
