...
void do_bgfg(char *argv, int bg, int output_fd)
{
    ...
    if (bg)
    {
        //dprintf(output_fd, "[%d] (%d) %s\n", job->jid, pid, job->cmdline); also okay
...
eval(char *cmdline) 
{
    ...
    switch (tok.builtins)
    {
        case BUILTIN_NONE:
        {
            ...
            sigset_t mask_all, mask, prev_one, waitmask;
        ...
        default:
            ...
            (void) fprintf(stderr, "Error: UNKNOWN type in builtin_cmd() function\n");
            exit(1);
    }
